...
def build_and_train():
    print("STARTING SCiKIT-LEARN ENSEMBLE PIPELINE (No XGBoost needed!)")
    ...
    for row in clean_records:
        date, p1, p2, surface = row["date"], row["player1"], row["player2"], row["surface"]
        ...
        p1_rest = (date - last_match_date.get(p1, date)).days
        p2_rest = (date - last_match_date.get(p2, date)).days
    ...
    print("Training the Ultimate AI Ensemble (Random Forest + Gradient Boost)...")
    
    # Expert 1: The Random Forest
    rf_expert = RandomForestClassifier(n_estimators=100, max_depth=7, random_state=42)
    
    # Expert 2: The Native Scikit-Learn Gradient Booster
...
if __name__ == "__main__":
    ...
    if artifacts:
        print("Saving Ensemble artifacts to disk...")
