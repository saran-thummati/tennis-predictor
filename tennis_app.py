...
# --- 1. THE BOUNCER (SESSION STATE) ---
# If this is the user's first time on the site, set their status to "Not Logged In"
if "logged_in" not in st.session_state:
    st.session_state.logged_in = False

# --- 2. THE LOGIN PAGE ---
if not st.session_state.logged_in:
    st.set_page_config(page_title="Login - Tennis Predictor", layout="centered")
    st.title("Welcome to the Tennis Predictor")
    st.markdown("Please log in to access the AI.")
    ...
    if st.button("Log In", use_container_width=True):
        ...
        if username == "admin" and password == "tennis123":
            st.session_state.logged_in = True
            st.rerun() # This instantly refreshes the page to show the app
        else:
            st.error("Incorrect username or password.")

# --- 3. THE MAIN APP (Only runs if they pass the Bouncer) ---
else:
    @st.cache_resource
    ...
    # We can add a Logout button to the sidebar!
    with st.sidebar:
