...
try:
    ...
    surface_form = artifacts.get("surface_form", {}) # Upgraded!
...
st.set_page_config(page_title="Elite AI Tennis Predictor", page_icon="🏆", layout="centered")
st.title("🏆 Elite AI Tennis Predictor")
st.markdown("Powered by a Hyperparameter-Tuned Ensemble, Surface-Specific Form, and Grand Slam Weighting.")
st.divider()
...
if st.button("🔮 Predict Match Outcome", use_container_width=True):
    if p1 == p2:
        ...
        with st.spinner("Calculating Optimized Probabilities..."):
            ...
            st.subheader("Optimized Ensemble Prediction")
            if p1_win_prob > 0.5:
            ...
            st.subheader("📊 Elite Evaluation Matrix")
            ...
            with c2:
                st.markdown(f"**Recent Form on {surface}**") # Dynamic Label!
                st.write(f"**{p1}:** {form_1_surf:.1f}% win rate")
                st.write(f"**{p2}:** {form_2_surf:.1f}% win rate")
            with c3:
