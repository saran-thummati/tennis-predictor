...
st.markdown("Powered by a Voting Classifier Ensemble (Random Forest + XGBoost), Elo Ratings, and Biometrics.")
st.divider()

col1, col2 = st.columns(2)
with col1:
    p1 = st.selectbox("Select First Player", all_players, key="p1")
    p1_rest = st.slider(f"Days Rested ({p1})", 0, 14, 2)
with col2:
    default_p2_idx = 1 if len(all_players) > 1 else 0
    p2 = st.selectbox("Select Second Player", all_players, index=default_p2_idx, key="p2")
    p2_rest = st.slider(f"Days Rested ({p2})", 0, 14, 2)
...
if st.button("🔮 Predict Match Outcome", use_container_width=True):
    if p1 == p2:
        ...
        with st.spinner("Board of Directors analyzing data..."):
            ...
            # Build Feature Array exact same way as training
            features = np.array([[
                r1_base - r2_base, 
                r1_surf - r2_surf, 
                h2h_adv, 
